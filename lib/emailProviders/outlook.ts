/**
 * Outlook SMTP Email Provider
 * Uses app password authentication (not regular Outlook password)
 */
...
class OutlookProvider implements EmailProvider {
  ...
  async createTransport(): Promise<nodemailer.Transporter> {
    ...
    if (!emailUser || !emailPassword) {
      throw new Error('EMAIL_USER and EMAIL_PASSWORD environment variables are required for Outlook');
    }
